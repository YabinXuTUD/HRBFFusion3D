...
//distortion-based
float confidence(float x, float y, float max_dist, float weighting)
{
    //float maxRadDist = max_dist; //sqrt((width * 0.5)^2 + (height * 0.5)^2)
    const float twoSigmaSquared = 0.72; //2*(0.6^2) from paper

    vec2 pixelPosCentered = vec2(x, y) - cam.xy;
    float radialDist = sqrt(dot(pixelPosCentered, pixelPosCentered)) / max_dist;
